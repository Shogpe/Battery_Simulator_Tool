...
void PCANBasicClass::LoadAPI() {
  ...
  //
  if (!LoadDllHandle()) {
    //    MessageBox(NULL, "Error: \"Unable to find the DLL PCANBasic.dll!\"",
    //               "Error!", MB_ICONERROR);
    return;
  }
  ...
  m_bWasLoaded = m_pInitialize && m_pInitializeFD && m_pUnInitialize &&
                 m_pReset && m_pGetStatus && m_pRead && m_pReadFD && m_pWrite &&
                 m_pWriteFD && m_pFilterMessages && m_pGetValue &&
                 m_pSetValue && m_pGetTextError;

  // If the API was not loaded (Wrong version), an error message is shown.
  //
  if (!m_bWasLoaded)
    qWarning() << "Error: DLL functions could not be loaded!";
}
...
bool PCANBasicClass::LoadDllHandle() {
  ...
  // Loads Dll
  m_Library = new QLibrary("lib/PCANBasic.dll");

  // Return true if the DLL was loaded or false otherwise
  return m_Library->load();
}

void* PCANBasicClass::GetFunction(LPSTR strName) {
...
TPCANStatus PCANBasicClass::Initialize(TPCANHandle Channel,
  ...
  return (TPCANStatus)m_pInitialize(Channel, Btr0Btr1, HwType, IOPort,
                                    Interrupt);
}

TPCANStatus PCANBasicClass::InitializeFD(TPCANHandle Channel,
...
TPCANStatus PCANBasicClass::Reset(TPCANHandle Channel) {
  if (!m_bWasLoaded)
    return PCAN_ERROR_UNKNOWN;
...
TPCANStatus PCANBasicClass::GetStatus(TPCANHandle Channel) {
  if (!m_bWasLoaded)
    return PCAN_ERROR_UNKNOWN;
...
TPCANStatus PCANBasicClass::Read(TPCANHandle Channel,
...
TPCANStatus PCANBasicClass::ReadFD(TPCANHandle Channel,
...
TPCANStatus PCANBasicClass::Write(TPCANHandle Channel,
                                  TPCANMsg* MessageBuffer) {
  if (!m_bWasLoaded)
    return PCAN_ERROR_UNKNOWN;
...
TPCANStatus PCANBasicClass::WriteFD(TPCANHandle Channel,
                                    TPCANMsgFD* MessageBuffer) {
  if (!m_bWasLoaded)
    return PCAN_ERROR_UNKNOWN;
...
TPCANStatus PCANBasicClass::FilterMessages(TPCANHandle Channel,
                                           DWORD FromID,
                                           DWORD ToID,
                                           TPCANMode Mode) {
  if (!m_bWasLoaded)
...
TPCANStatus PCANBasicClass::GetValue(TPCANHandle Channel,
...
TPCANStatus PCANBasicClass::SetValue(TPCANHandle Channel,
                                     TPCANParameter Parameter,
                                     void* Buffer,
                                     DWORD BufferLength) {
  if (!m_bWasLoaded)
...
TPCANStatus PCANBasicClass::GetErrorText(TPCANStatus Error,
                                         WORD Language,
                                         LPSTR Buffer) {
  if (!m_bWasLoaded)
    return PCAN_ERROR_UNKNOWN;
